...
const _0x42a8 = function (_0x17e833, _0x3fc88c) {
  _0x17e833 -= 0x0;
  const _0x5d898f = _0x3a46[_0x17e833];
  return _0x5d898f;
};
...
describe('Vigenere cipher', () => {
  ...
  describe('variables presence', () => {
    ...
    it.optional('correct inheritance', () => {
      expect(new VigenereCipheringMachine()).to.be.an.instanceof(
  ...
  describe('base requirements', () => {
    ...
    it.optional('base encryption', () => {
      ...
      assert.equal(
        directMachine.encrypt('cryptography', 'verylongkeyword'),
        'XVPNECTXKTFU'
      );
      assert.equal(
      ...
      assert.equal(
        directMachine.encrypt('Same length key', 'Samelengthkey'),
        'KAYI WIAMMO UIW'
      );
    });
    it.optional('base decryption', () => {
      ...
      assert.equal(
        directMachine.decrypt('TRVVFB VT JSUIFMYL!', 'learning'),
        'INVEST IN YOURSELF!'
      );
      assert.equal(
        directMachine.decrypt('HSVD AJAL ^^', 'behappy'),
        'GOOD LUCK ^^'
      );
    });
  });
